...
namespace tRW.Content.BugSwarms.Bugs
{
    public class CoolEffect
    {
        ...
        public int oldPosLength = 7;
        public Vector2[] oldPositions = new Vector2[7];
        ...
        {
            float influenceDistance = flyPosition.Distance(swarmPosition) / 64;
            ...
            if (timer % 4 == 0)
            {
                heading += Main.rand.NextFloat(MathHelper.ToRadians(-60), MathHelper.ToRadians(60));
                direction = heading.ToRotationVector2();
                velocity = (influence + direction) * speed;
            }
