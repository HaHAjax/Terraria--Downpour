...
public class CoolEffectSwarm : ModNPC
{
    public override string Texture => "tRW/Assets/NPCs/FlySwarm";

    private int maxFlies = 50;
    private CoolEffect[] flies = new CoolEffect[50];
    ...
    public override bool PreDraw(SpriteBatch spriteBatch, Vector2 screenPos, Color drawColor)
    {
        for (int i = 0; i < maxFlies; i++)
        {
            ...
            else
            {
                ...
                Texture2D baseTexture = ModContent.Request<Texture2D>("tRW/Assets/NPCs/CoolEffect", AssetRequestMode.ImmediateLoad).Value;
                ...
                for (int k = 0; k < flies[i].oldPositions.Length; k++)
                {
                    ...
                    //Color color = new(94, 160, 255, 125);
                    Color color = (Color)GetAlpha(drawColor) * ((flies[i].oldPositions.Length - k) / (float)flies[i].oldPositions.Length); // fix this later
                    Vector2 scale = new(((flies[i].flyPosition - flies[i].oldPositions[k]).Length() / baseTexture.Width) / 1.1f, 0.225f);
    ...
    public override Color? GetAlpha(Color drawColor) // this needs to return something (look in ExampleBullet)
    {
